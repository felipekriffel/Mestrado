...
\begin{frame}{4.2 - Dist. condicional de 2ª camada} 
    ...
    \begin{equation}
        p(z^{(2)} \mid z^{(1)}) = \frac{1}{|2\pi \hat G^{(2)}|^{\frac{n_2}{2}}} \exp \left[- \frac{1}{2} \sum_{j, \alpha_1, \alpha_2}^{n_2} \hat G_{(2)}^{\alpha_1 \alpha_2} z^{(2)}_{j; \alpha_1} z^{(2)}_{j; \alpha_2}\right]
    \end{equation} 
    \pause
    onde, definimos a \textbf{métrica \textit{estocástica} de segunda camada}.
    \begin{equation}
        \hat G_{\alpha_1\alpha_2}^{(2)} := C_b^{(2)} + C_W^{(2)} \frac{1}{n_1} \sum_{j=1}^{n_1} x_{j; \alpha_1} x_{j; \alpha_2}
    \end{equation}
\end{frame}
