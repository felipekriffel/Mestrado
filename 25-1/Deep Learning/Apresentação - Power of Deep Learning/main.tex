...
%     Definimos os custos de achatamento por contagem de neurônios e por sinapses, respectivamente, como:

%     \begin{align*}
%         C_n(f,l,\varepsilon) = \min_{f^l_\varepsilon} \frac{N_n(f^l_\varepsilon)}{N_n(f)} \\
%         C_s(f,l,\varepsilon) = \min_{f^l_\varepsilon} \frac{N_s(f^l_\varepsilon)}{N_s(f)}
%     \end{align*}.
% \end{frame}

% \begin{frame}{Custo de Achatamento}
%     \textbf{Definição:} Se $f$ é rede neural, dizemos que uma rede $f^l_\varepsilon$ é sua versão achatada se tem um número $l$ menor de camadas que $f$ e aproxima $f$ a um erro $\varepsilon$ (para uma distância estabelecida).
    
%     \vspace{1em}
    
%     I.e., se $f = \sigma_k A_k \cdots \sigma_1 A_1$, $f_\varepsilon^l=\eta_l A_l \cdots \eta_1 A_1$ temos $l<k$ e $|f(\mathbf x) - f_\varepsilon^l(\mathbf x)|<\varepsilon$, para todo $x \in D(f)$.

%     \vspace{1em}
% \end{frame}

% \begin{frame}{Custo de Achatamento}
%     \textbf{Definição:} Chamamos o número de neurônios $N_n(f)$ de uma rede $f$ a soma das dimensões de suas camadas escondidas. I.e, se $f = \sigma_k A_k \cdots \sigma_1 A_1$ e $A_i : \mathbb R^{n_i} \to \mathbb R^{m_i}$, p/ $i=1, \dots, k$, então

%     \[
%         N_n(f) = n_1 + \sum_{i=1}^k m_i.
%     \]

%     \textbf{Definição:} Chamamos o número de sinapses $N_s(f)$ de uma rede $f$ o número de entradas não-nulas das matrizes de pesos $A_i$.

%     \pause

%     \textbf{Definição:} Dizemos que $\mathbf f^l_\varepsilon$ é um achatamento neurônio-eficiente de $f$ se $N_n(f_\varepsilon^l)<N_n(f)$.
    
%     \vspace{1em}
    
%     \textbf{Definição:} Dizemos que $\mathbf f^l_\varepsilon$ é um achatamento sinapse-eficiente de $f$ se $N_s(f_\varepsilon^l)<N_s(f)$.
